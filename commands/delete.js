...
async function deleteCommand(sock, chatId, message, senderId) {
    try {
        ...
        if (!isBotAdmin) {
            await sock.sendMessage(chatId, { text: 'Necesito ser administrador para eliminar mensajes.' }, { quoted: message });
            return;
        }

        if (!isSenderAdmin) {
            await sock.sendMessage(chatId, { text: 'Solo los administradores pueden usar el comando .delete.' }, { quoted: message });
            return;
        }
        ...
        // If no number provided and not replying/mentioning, show usage message
            ...
            await sock.sendMessage(chatId, { 
                text: '❌ Por favor, especifica el número de mensajes a eliminar.\n\nUso:\n• `.del 5` - Elimina los últimos 5 mensajes del grupo\n• `.del 3 @usuario` - Elimina los últimos 3 mensajes de @usuario\n• `.del 2` (respondiendo a un mensaje) - Elimina los últimos 2 mensajes del usuario al que se responde' 
            }, { quoted: message });
            return;
        }
        ...
        if (toDelete.length === 0) {
            const errorMsg = deleteGroupMessages 
                ? 'No se encontraron mensajes recientes en el grupo para eliminar.' 
                : 'No se encontraron mensajes recientes para este usuario.';
        ...
        await sock.sendMessage(chatId, { text: 'Falló al intentar eliminar los mensajes.' }, { quoted: message });
    }
}
